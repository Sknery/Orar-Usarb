                    ...
                    {weekDays.map(day => (
                        <TableRow key={day.toISOString()} className={cn("cursor-pointer", isSameDay(day, selectedDate) && "bg-muted/50")} onClick={() => onDaySelect(day)}>
                            ...
                            {timeSlots.map((slot) => {
                                ...
                                return <TableCell key={slot} className={cn("text-center text-xs p-1 font-bold", lesson && "bg-primary/20 rounded-md")}>{lesson ? getLessonAbbreviation(lesson.type) : '-'}</TableCell>;
                            })}
                        </TableRow>
                    ))}
                </TableBody>
...
function App() {
  ...
  return (
    <main className="dark bg-background text-foreground h-screen w-screen overflow-hidden p-2 sm:p-4 flex flex-col">
      <header className="flex-shrink-0 mb-2 px-2 sm:px-0 flex items-center justify-center sm:justify-start gap-3 mt-2 sm:mt-0">
      ...
      {isDesktop ? (
        ...
        <div className="flex-grow min-h-0 relative overflow-hidden">
          <AnimatePresence>
            {mobileView === 'main' && (
              <motion.div {...bindMainViewDrag()} key="main" initial={{ x: isInitialLoad ? '0%' : '-100%' }} animate={{ x: '0%' }} exit={{ x: '-100%' }} transition={{ duration: 0.3, ease: 'easeInOut' }} className="absolute top-0 left-0 w-full h-full flex flex-col gap-2 touch-none">
